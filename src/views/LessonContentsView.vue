...
import { Plus, Edit, Trash2, ChevronLeft, ChevronRight, PlayCircle, Upload, X, FileText } from 'lucide-vue-next'
...
const form = reactive({
  ...
  content: '',
  mediaUrl: '',
  mediaType: 'none', // 'upload', 'url', 'youtube', 'none'
  mediaFile: null,
  order_number: 1,
  lessonId: '',
  isActive: true
    ...
    <div class="mb-6">
      <div class="flex justify-between items-center">
        <div>
          ...
          <p class="text-muted-foreground">Manage video and text content for lessons</p>
        </div>
      ...
      <div class="px-6">
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div class="space-y-2">
            ...
            <Select v-model="selectedUnit" :disabled="!selectedCourse || selectedCourse === 'all'">
              <SelectTrigger :disabled="!selectedCourse || selectedCourse === 'all'">
          ...
          <div class="space-y-2">
            ...
            <Select v-model="selectedLesson" :disabled="!selectedUnit || selectedUnit === 'all'">
              <SelectTrigger :disabled="!selectedUnit || selectedUnit === 'all'">
    ...
    <Card>
      <Table>
        ...
        <TableBody>
          ...
          <TableRow v-else v-for="content in paginatedContents" :key="content.id">
            <TableCell>
              <div>
                <div class="font-medium">{{ content.title }}</div>
                <div class="text-sm text-muted-foreground max-w-xs truncate">{{ content.content }}</div>
              </div>
            </TableCell>
            <TableCell>
              <div>{{ getCourseNameById(content.lessonId) }}</div>
              <div class="text-sm text-muted-foreground">{{ getUnitNameById(content.lessonId) }} > {{ getLessonNameById(content.lessonId) }}</div>
            </TableCell>
            <TableCell>
              <div v-if="content.mediaUrl">
                <a :href="content.mediaUrl" target="_blank" class="text-primary hover:underline flex items-center">
    ...
    <Dialog :open="showModal" @update:open="(val) => { if (!val) closeModal() }">
      <DialogContent class="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {{ isEditMode ? 'Update the lesson content' : 'Add new content to a lesson' }}
          </DialogDescription>
        </DialogHeader>
        ...
        <form @submit.prevent="saveContent" class="space-y-4">
          ...
          <div class="space-y-2">
            ...
            <!-- Course Selection -->
            <div class="space-y-2">
              <Label for="course" class="text-sm text-muted-foreground">Course</Label>
              <Select v-model="selectedCourse">
            ...
            <!-- Unit Selection -->
            <div class="space-y-2">
              <Label for="unit" class="text-sm text-muted-foreground">Unit</Label>
              <Select v-model="selectedUnit" :disabled="!selectedCourse || selectedCourse === 'all'">
                <SelectTrigger id="unit" :disabled="!selectedCourse || selectedCourse === 'all'">
            ...
            <!-- Lesson Selection -->
            <div class="space-y-2">
              <Label for="lessonSelect" class="text-sm text-muted-foreground">Select Lesson</Label>
              <Select v-model="form.lessonId" :disabled="!selectedUnit || selectedUnit === 'all'" required>
                <SelectTrigger id="lessonSelect" :disabled="!selectedUnit || selectedUnit === 'all'">
          ...
          <div v-if="form.mediaType === 'upload'" class="space-y-2">
            ...
            >
              ...
              <!-- Selected File Info -->
              <div v-else-if="form.mediaFile && !isUploading" class="bg-green-50 dark:bg-green-900/20 rounded-lg p-3">
                <div class="flex items-center justify-between">
                  <div class="flex items-center">
                    <FileText class="h-5 w-5 text-green-600 dark:text-green-400 mr-2" />
                    <div class="text-left">
                      <p class="text-sm font-medium text-green-800 dark:text-green-200">{{ form.mediaFile.name }}</p>
                      <p class="text-xs text-green-600 dark:text-green-400">{{ (form.mediaFile.size / 1024 / 1024).toFixed(2) }} MB</p>
                    </div>
                  </div>
                  <Button
                    type="button"
                    variant="ghost"
                    size="icon"
                    @click.stop="form.mediaFile = null; $refs.fileInput.value = ''"
                  >
              ...
              <div v-else-if="isUploading" class="space-y-3">
                <div class="flex items-center justify-center">
                  ...
                  <span class="text-sm">Uploading... {{ uploadProgress }}%</span>
                </div>
                <Progress :model-value="uploadProgress" class="w-full" />
                <p class="text-xs text-muted-foreground">{{ form.mediaFile?.name }}</p>
              </div>
            </div>
          </div>

          <!-- YouTube URL -->
          <div v-if="form.mediaType === 'youtube'" class="space-y-2">
            ...
            <div v-if="youtubePreview" class="mt-2 p-2 bg-muted rounded border">
              ...
              <div class="aspect-video bg-muted-foreground/10 rounded flex items-center justify-center">
          ...
          <DialogFooter>
            ...
            >
              <div v-if="isLoading || isUploading" class="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
