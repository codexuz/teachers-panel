...
// API Base URL
const API_BASE_URL = 'http://backend.impulselc.uz/api'
...
const booksAPI = {
  ...
  update: (id, data) => apiRequest(`/books/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
...
const moviesAPI = {
  ...
  update: (id, data) => apiRequest(`/movies/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
...
const videosAPI = {
  ...
  create: (data) => apiRequest('/videos', { method: 'POST', body: JSON.stringify(data) }),
  update: (id, data) => apiRequest(`/videos/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
...
const getLevelColor = (level) => {
  switch (level) {
    case 'A1': case 'A2': return 'green'
    case 'B1': case 'B2': return 'yellow'
    case 'C1': case 'C2': return 'red'
    default: return 'gray'
  }
}
  ...
  <div class="p-6">
    ...
    <div class="mb-6">
      <div class="flex justify-between items-center">
        ...
        >
          <i class="fas fa-plus mr-2"></i>Add {{ activeTab.charAt(0).toUpperCase() + activeTab.slice(1, -1) }}
        </button>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      <div class="bg-white rounded-lg shadow p-6 text-center">
        <div class="p-4 bg-blue-100 rounded-full mx-auto mb-4 w-16 h-16 flex items-center justify-center">
      ...
      <div class="bg-white rounded-lg shadow p-6 text-center">
        <div class="p-4 bg-purple-100 rounded-full mx-auto mb-4 w-16 h-16 flex items-center justify-center">
          <i class="fas fa-film text-purple-600 text-2xl"></i>
        </div>
        <h3 class="text-2xl font-bold text-gray-800">{{ totalStats.movies }}</h3>
        <p class="text-gray-600">Movies</p>
      </div>

      <div class="bg-white rounded-lg shadow p-6 text-center">
        <div class="p-4 bg-red-100 rounded-full mx-auto mb-4 w-16 h-16 flex items-center justify-center">
          <i class="fas fa-play text-red-600 text-2xl"></i>
        </div>
        <h3 class="text-2xl font-bold text-gray-800">{{ totalStats.videos }}</h3>
        <p class="text-gray-600">Videos</p>
      </div>

      <div class="bg-white rounded-lg shadow p-6 text-center">
        <div class="p-4 bg-green-100 rounded-full mx-auto mb-4 w-16 h-16 flex items-center justify-center">
    ...
    <div class="bg-white rounded-lg shadow">
      ...
      <div class="divide-y divide-gray-200">
        <!-- Loading State -->
        <div v-if="isLoading && currentItems.length === 0" class="p-6 text-center">
        ...
        <div v-else-if="currentItems.length === 0" class="p-12 text-center">
          <i :class="`fas ${getItemIcon(activeTab)}`" class="text-gray-400 text-4xl mb-4"></i>
          <h3 class="text-lg font-medium text-gray-900 mb-2">No {{ activeTab }} found</h3>
        ...
        <!-- Items List -->
        <div v-else v-for="item in currentItems" :key="item.id" class="p-6 hover:bg-gray-50">
          <div class="flex items-center justify-between">
            <div class="flex items-center">
              <div :class="`p-2 bg-${getItemColor(activeTab)}-100 rounded-full`">
                <i :class="`fas ${getItemIcon(activeTab)} text-${getItemColor(activeTab)}-600`"></i>
              </div>
              <div class="ml-4">
                <h3 class="text-lg font-semibold text-gray-800">{{ item.title || item.url }}</h3>
                ...
                <div class="flex items-center mt-2 text-sm text-gray-500 space-x-4">
                  <span v-if="item.level" :class="`px-2 py-1 bg-${getLevelColor(item.level)}-100 text-${getLevelColor(item.level)}-800 rounded-full`">
                    {{ item.level }}
                  </span>
      ...
      <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
          ...
          <form @submit.prevent="saveItem" class="space-y-4">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">
                {{ activeTab === 'videos' ? 'YouTube URL' : 'URL' }} <span class="text-red-500">*</span>
              </label>
              <input
                type="url"
                v-model="form.url"
