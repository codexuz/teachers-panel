...
const totalPages = computed(() => Math.ceil(filteredSpeakingExercises.value.length / itemsPerPage.value))
...
const fetchSpeakingExercises = async () => {
  try {
    ...
    speakingExercises.value = response || []
  } catch (err) {
...
const submitForm = async () => {
  try {
    ...
    error.value = isEditMode.value ? 'Failed to update speaking exercise' : 'Failed to create speaking exercise'
...
const deleteSpeakingExercise = async (id) => {
  if (!confirm('Are you sure you want to delete this speaking exercise?')) return
  ...
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
      <div>
        ...
        <p class="text-muted-foreground">Manage speaking practice exercises and conversation prompts</p>
      </div>
          ...
          <div class="space-y-2">
            ...
            <Select v-model="selectedUnit" :disabled="!selectedCourse">
              ...
              <SelectContent>
                ...
                <SelectItem v-for="unit in filteredUnits" :key="unit.id" :value="unit.id">
                  {{ unit.title }}
                </SelectItem>
              </SelectContent>
            </Select>
          ...
          <div class="space-y-2">
            ...
            <Select v-model="selectedLesson" :disabled="!selectedUnit">
              ...
              <SelectContent>
                ...
                <SelectItem v-for="lesson in filteredLessons" :key="lesson.id" :value="lesson.id">
                  {{ lesson.title }}
                </SelectItem>
              </SelectContent>
            </Select>
    ...
    <Card>
      <CardHeader>
        <CardTitle>Speaking Exercises ({{ filteredSpeakingExercises.length }})</CardTitle>
      </CardHeader>
      <CardContent>
        ...
        <div v-else-if="filteredSpeakingExercises.length === 0" class="text-center py-8">
          <Mic class="mx-auto w-12 h-12 text-gray-400 mb-4" />
          <p class="text-lg font-medium text-gray-900">No speaking exercises found</p>
          <p class="text-gray-600">Create your first speaking exercise to get started.</p>
        </div>
        ...
        <div v-else>
          ...
          <div class="hidden md:block">
            <Table>
              ...
              <TableBody>
                <TableRow v-for="exercise in paginatedSpeakingExercises" :key="exercise.id">
                  ...
                  <TableCell class="text-right">
                    <div class="flex items-center justify-end gap-2">
                      <Button 
                        variant="outline" 
                        size="sm" 
                        @click="exercise.type === 'pronunciation' ? $router.push(`/pronunciation/${exercise.id}`) : $router.push(`/speaking/${exercise.id}`)" 
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuSeparator />
                      <DropdownMenuItem @click="exercise.type === 'pronunciation' ? $router.push(`/pronunciation/${exercise.id}`) : $router.push(`/speaking/${exercise.id}`)">
          ...
          <div class="md:hidden">
            <div class="grid gap-4 sm:grid-cols-2">
              <Card v-for="exercise in paginatedSpeakingExercises" :key="exercise.id" class="hover:shadow-md transition-shadow">
                <CardHeader>
          ...
          <div class="flex items-center justify-between space-x-2 py-4" v-if="totalPages > 1">
            <div class="text-sm text-muted-foreground">
              ...
              {{ Math.min(currentPage * itemsPerPage, filteredSpeakingExercises.length) }} of 
              {{ filteredSpeakingExercises.length }} results
            </div>
    ...
    <Dialog :open="showModal" @update:open="closeModal">
      <DialogContent class="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{{ isEditMode ? 'Edit' : 'Create' }} Speaking Exercise</DialogTitle>
          <DialogDescription>
            {{ isEditMode ? 'Update the speaking exercise details.' : 'Create a new speaking exercise for language practice.' }}
          </DialogDescription>
        </DialogHeader>
        ...
        <form @submit.prevent="submitForm" class="space-y-6">
          <div class="space-y-4">
            ...
            <div class="space-y-2">
              ...
              <Select v-model="form.type" required>
                ...
                <SelectContent>
                  <SelectItem v-for="type in speakingTypes" :key="type.value" :value="type.value">
            ...
            <div class="space-y-2">
              ...
              <Select v-model="form.unitId" :disabled="!form.courseId" required>
                ...
                <SelectContent>
                  <SelectItem 
                    v-for="unit in units.filter(u => u.courseId === form.courseId)" 
                    :key="unit.id" 
                    :value="unit.id"
                  >
                    {{ unit.title }}
            ...
            <div class="space-y-2">
              ...
              <Select v-model="form.lessonId" :disabled="!form.unitId" required>
                ...
                <SelectContent>
                  <SelectItem 
                    v-for="lesson in lessons.filter(l => l.moduleId === form.unitId)" 
                    :key="lesson.id" 
                    :value="lesson.id"
                  >
                    {{ lesson.title }}
          ...
          <DialogFooter>
            <Button type="button" variant="outline" @click="closeModal" :disabled="isLoading">
              Cancel
            </Button>
