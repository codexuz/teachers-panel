...
// Lucide Icons
import { ArrowLeft, Plus, Users, CalendarCheck, Trash2, X, User, Phone, RefreshCw, RotateCcw } from 'lucide-vue-next'
...
<template>
  <div class="flex flex-col h-full">
    ...
    <div class="p-4 sm:p-6 border-b bg-background">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-2xl sm:text-3xl font-bold tracking-tight">{{ groupName }}</h1>
    ...
    <div class="flex-1 p-4 sm:p-6">
      <Tabs :model-value="activeTab" @update:model-value="activeTab = $event" class="h-full flex flex-col">
        <TabsList class="grid w-fit grid-cols-2 h-9">
          <TabsTrigger value="students" class="flex items-center gap-1 px-3 py-1 text-sm">
            <Users class="h-3 w-3" />
            <span class="hidden sm:inline">Students</span>
            <Badge variant="secondary" class="ml-1 text-xs px-1 py-0">{{ students.length }}</Badge>
          </TabsTrigger>
          <TabsTrigger value="attendance" class="flex items-center gap-1 px-3 py-1 text-sm">
          ...
          <div class="space-y-6">
            <div class="flex justify-between items-center">
              ...
              <Dialog>
                <DialogTrigger asChild>
                  <Button @click="openAddStudentModal" class="flex items-center gap-2">
                    <Plus class="h-4 w-4" />
                    Add Student
                  </Button>
                </DialogTrigger>
            ...
            <Card v-else-if="students.length === 0" class="text-center py-12">
              <CardContent class="pt-6">
                ...
                <CardDescription class="mb-4">Add your first student to this group</CardDescription>
                <Button @click="openAddStudentModal" class="flex items-center gap-2 mx-auto">
                  <Plus class="h-4 w-4" />
                  Add Student
                </Button>
              </CardContent>
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <Card v-for="student in students" :key="student.id" class="hover:shadow-md transition-shadow">
                <CardContent class="p-4">
                  <div class="flex items-center justify-between mb-3">
                    <div class="flex items-center gap-3">
                      ...
                      <div>
                        <h4 class="font-medium">{{ student.first_name }}</h4>
                        <p class="text-sm text-muted-foreground">{{ student.last_name }}</p>
                      </div>
                    </div>
                    <Button
                      variant="ghost"
                  ...
                  <div class="space-y-2">
                    <div v-if="student.phone" class="flex items-center gap-2 text-sm text-muted-foreground">
                      <Phone class="h-3 w-3" />
                      {{ student.phone }}
                    </div>
                    ...
                    <div class="flex items-center justify-between">
                      ...
                      <div v-if="student.enrolled_at" class="text-xs text-muted-foreground">
              ...
              <CardHeader>
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    ...
                    <CardDescription>Track student attendance for this group</CardDescription>
                  </div>
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button @click="openCreateAttendanceModal" variant="default" class="flex items-center gap-2">
                        <Plus class="h-4 w-4" />
                        Create Attendance
                      </Button>
                    </DialogTrigger>
                ...
                <div class="flex flex-col sm:flex-row gap-4 mt-4">
                  ...
                  <div class="flex items-end">
                    <Button @click="fetchAttendance" variant="outline" class="flex items-center gap-2">
                      ...
                      <TableRow v-if="attendance.length === 0">
                        <TableCell colspan="4" class="text-center py-8">
                          <div class="flex flex-col items-center gap-2">
                            <CalendarCheck class="h-8 w-8 text-muted-foreground" />
      ...
      <DialogContent class="max-w-2xl max-h-[80vh]">
        ...
        <div class="space-y-4">
          ...
          <div class="text-sm text-muted-foreground">
            <Badge variant="secondary">{{ selectedStudentsToAdd.length }}</Badge> students selected
          </div>

          <!-- Students list -->
          <div class="border rounded-lg">
            <div v-if="isLoadingAllStudents" class="flex items-center justify-center py-8">
            ...
            <div v-else class="max-h-96 overflow-y-auto">
              <div 
                ...
                :class="{ 'bg-accent': isStudentSelected(student.user_id || student.id) }"
              >
                <Checkbox 
                  :checked="isStudentSelected(student.user_id || student.id)"
                  class="mr-3"
                />
                <Avatar class="mr-3">
                  <AvatarFallback>
                ...
                <div class="flex-1">
                  ...
                  <div v-if="student?.phone" class="text-sm text-muted-foreground flex items-center gap-1">
                    <Phone class="h-3 w-3" />
                    {{ student?.phone }}
                  </div>
                </div>
    ...
    <Dialog v-model:open="showAttendanceModal">
      <DialogContent class="max-w-2xl max-h-[80vh]">
        <DialogHeader>
          ...
          <DialogDescription>
            Mark attendance for all students in this group for the selected date.
          </DialogDescription>
        </DialogHeader>
                ...
                >
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div class="text-sm text-muted-foreground">{{ student.last_name || student.email }}</div>
                    </div>
                  </div>
                  
                  <RadioGroup 
                    :model-value="attendanceStatuses[student.user_id]" 
                    @update:model-value="attendanceStatuses[student.user_id] = $event"
                    class="flex gap-6"
                  >
                    <div class="flex items-center space-x-2">
                      <RadioGroupItem :value="'present'" :id="`present-${student.user_id}`" />
                      <Label :for="`present-${student.user_id}`" class="text-green-700">Present</Label>
                    </div>
                    <div class="flex items-center space-x-2">
                      <RadioGroupItem :value="'absent'" :id="`absent-${student.user_id}`" />
                      <Label :for="`absent-${student.user_id}`" class="text-red-700">Absent</Label>
                    </div>
                  </RadioGroup>
                </div>
              </div>
