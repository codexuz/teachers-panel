...
const PEXELS_API_KEY =
  "x9JPjTC9LmpxAkisngyQTO371TsvHiwnGeg9wX75RuXJFzJx5KfCn1bI";
...
const filteredItems = computed(() => {
  let filtered = vocabularyItems.value.filter((item) => {
    ...
    const matchesFilter =
      ...
      (filterType.value === "no-audio" && !item.audio_url) ||
      (filterType.value === "no-images" && !item.image_url);
...
const stats = computed(() => {
  ...
  const withAudio = vocabularyItems.value.filter(
    (item) => item.audio_url
  ).length;
  const withImages = vocabularyItems.value.filter(
    (item) => item.image_url
  ).length;
  const withExamples = vocabularyItems.value.filter(
    (item) => item.example
  ).length;
...
const searchImagesForWord = async (word) => {
  ...
  try {
    ...
    searchImagesError.value = `Error searching for images: ${
    ...
    const result = await uploadAPI.uploadFile(file, {
      onUploadProgress: (event) => {
        if (event.lengthComputable) {
          uploadAudioProgress.value = Math.round(
...
const generateOxfordAudio = (word) => {
  ...
  try {
    ...
    audio.addEventListener("error", () => {
      console.log(
        "Oxford audio not available for this word, trying TTS instead..."
      );
...
const exportToCSV = () => {
  ...
  link.setAttribute(
    "download",
    `vocabulary_${vocabularySet.value?.title || "items"}_${
  ...
  try {
    ...
    for (const item of items) {
      ...
      if ((successCount + failureCount) % 5 === 0) {
        console.log(
          `Import progress: ${successCount + failureCount}/${items.length}`
        );
      }
    }
    ...
    if (failureCount > 0) {
      alert(
        `Import completed with ${successCount} items imported successfully and ${failureCount} failed items.`
      );
    } else {
...
onMounted(() => {
  ...
  if (!PEXELS_API_KEY) {
    console.error(
      "Pexels API key is missing. Image search functionality may not work."
    );
  }
});
  ...
  <div id="vocabulary-items" class="content-page">
      ...
      <div class="flex justify-between items-center">
        <div>
          <h2 class="text-2xl font-bold">
            {{ vocabularySet?.title || "Vocabulary Items" }}
          </h2>
          <p class="text-muted-foreground">
            {{ vocabularySet?.description || "Manage vocabulary items" }}
          </p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      ...
      <Card>
        <CardContent class="p-6">
          <div class="flex items-center">
            <div
              class="p-3 rounded-full bg-purple-500/10 dark:bg-purple-500/20"
            >
      ...
      <Card>
        <CardContent class="p-6">
          <div class="flex items-center">
            <div
              class="p-3 rounded-full bg-yellow-500/10 dark:bg-yellow-500/20"
            >
        ...
        <div class="flex flex-col sm:flex-row gap-4">
          ...
          <Select v-model="filterType">
            ...
            <SelectContent>
              ...
              <SelectItem value="no-audio">No Audio</SelectItem>
        ...
        <p class="text-muted-foreground">
          {{
            searchQuery
              ...
              : "Add your first vocabulary item to get started"
          }}
        </p>
      </div>
        ...
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center flex-1">
              <div
                class="w-16 h-16 bg-muted rounded-lg flex items-center justify-center mr-4"
              >
                <img
                  v-if="item.image_url"
                  :src="item.image_url"
              ...
              <div class="flex-1">
                  ...
                  <div class="flex items-center gap-2">
                    <Button
                      ...
                      :title="
                        playingAudioUrl === item.audio_url
                          ? 'Stop audio'
                          : 'Play audio for ' + item.word
                      "
                    >
                      <Stop
                        v-if="playingAudioUrl === item.audio_url"
                        class="h-4 w-4"
                      />
                    ...
                    <Badge v-if="item.image_url" variant="secondary"
                      >Has Image</Badge
                    >
                ...
                <p
                  v-if="item.example"
                  class="text-sm text-muted-foreground mt-2 flex items-start gap-1"
                >
            ...
            <div class="flex gap-2 ml-4">
              <Button @click="editVocabularyItem(item)" size="sm" class="gap-1">
                Edit
              </Button>
              <Button
      ...
      >
        <div class="flex items-center justify-between">
          <div class="text-sm text-muted-foreground">
            ...
            {{ filteredItems.length }} vocabulary items
          </div>
          <div class="flex gap-2">
            <Button
              @click="previousPage"
    ...
    <Dialog :open="showModal" @update:open="(open) => !open && closeModal()">
      <DialogContent class="max-w-5xl max-h-[90vh] overflow-y-auto">
              ...
              <div class="space-y-2">
                <div class="flex gap-2">
                  <div class="flex-1 relative">
                    <Input
                      ...
                      type="url"
                      :class="[
                        audioValidationStatus[itemForm.audio_url?.trim()] ===
                        'valid'
                          ? 'border-green-500'
                          : audioValidationStatus[
                              itemForm.audio_url?.trim()
                            ] === 'error'
                          ? 'border-destructive'
                          : '',
                      ]"
                  ...
                  >
                    <Stop
                      v-if="playingAudioUrl === itemForm.audio_url"
                      class="h-4 w-4"
                    />
                ...
                <div class="flex gap-2">
                  ...
                  <label
                    class="flex-1"
                    :class="{
                      'opacity-70 pointer-events-none': isUploadingAudio,
                    }"
                  >
                    <input
                      type="file"
                ...
                <Alert>
                  ...
                  <AlertDescription class="text-xs">
                    Supported audio formats: MP3, WAV, OGG, M4A, and AAC. Max
                    file size: 10MB.
                  </AlertDescription>
                </Alert>
              </div>
              ...
              <div v-if="searchImages.length > 0" class="mt-4 space-y-2">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  <div
                    ...
                    :class="{
                      'ring-2 ring-primary': itemForm.image_url === image.url,
                    }"
                  >
                    <img
                      :src="image.url"
            ...
            <CardContent class="space-y-2 text-sm">
              ...
              <div v-if="itemForm.image_url" class="flex items-center gap-2">
                ...
                <img
                  ...
                  @load="
                    () =>
                      console.log('Preview section image loaded successfully')
                  "
                  @error="
                    (e) => {
                      console.log(
                        'Preview section image failed to load:',
                        itemForm.image_url
                      );
        ...
        <DialogFooter>
          <Button @click="closeModal" type="button" variant="outline">
            Cancel
          </Button>
          <Button @click="submitVocabularyItem" :disabled="submitting">
    ...
    <Dialog
      :open="showBulkModal"
      @update:open="(open) => !open && closeBulkModal()"
    >
      <DialogContent class="max-w-2xl">
        ...
        <div class="space-y-6">
          <div>
            ...
            >
              ...
              <p
                v-if="csvFile"
                class="text-sm text-green-600 mb-2 flex items-center justify-center gap-1"
              >
        ...
        <DialogFooter>
          <Button @click="closeBulkModal" variant="outline"> Cancel </Button>
          <Button
            @click="importCSV"
